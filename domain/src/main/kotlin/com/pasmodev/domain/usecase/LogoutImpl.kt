...
class LogoutImpl(private var authRepository: AuthRepository) : Logout {

    override fun invoke(onComplete: (error: Error?) -> Unit) {
        if (!authRepository.logged){
            onComplete(Error("Authentication Required", AuthRequiredException("Authentication Required")))
            return
        }
